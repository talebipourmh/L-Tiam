    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        if (convertView == null) {
            convertView = LayoutInflater.from(context)
                    .inflate(R.layout.item_donation_marketplace, parent, false);
        }
        ...
        boolean available = "AVAILABLE".equalsIgnoreCase(itemStatus)
        ...
        request.setOnClickListener(v -> {
            if (requestListener != null && request.isEnabled()) requestListener.onRequest(item);
        });
